...
Rectangle {
    ...
    RowLayout {
        ...
        Item {
            ...
            property real value: section.settingsManager ? section.settingsManager.yuraOrbRestSeconds : 5
            ...
            Rectangle {
                ...
                Rectangle {
                    ...
                    width: parent.width * (slider.value - slider.from) / (slider.to - slider.from)
            ...
            Rectangle {
                ...
                height: 16
                radius: 8
                color: section.theme ? Qt.rgba(section.theme.accent.r, section.theme.accent.g, section.theme.accent.b, sliderMouseArea.pressed ? 1.0 : 0.95) : Qt.rgba(0.65, 0.55, 0.85, 0.95)
                border.width: 1
            ...
            MouseArea {
                ...
                onPressed: (mouse) => {
                    slider.value = slider.valueAt(mouse.x)
                    section.bump()
                }
                onPositionChanged: (mouse) => {
                    if (pressed) slider.value = slider.valueAt(mouse.x)
                }
                onReleased: {
                    if (section.settingsManager) {
                        section.settingsManager.yuraOrbRestSeconds = Math.round(slider.value)
        ...
        Text {
            ...
            text: Math.round(slider.value) === 0 ? "Always" : Math.round(slider.value) + "s"
